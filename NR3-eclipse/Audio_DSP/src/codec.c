...
void codec_ctrl_init()
{
	...
	CodecCommandBuffer[1] = 0xAF; //Headphone On , Speaker OFF
	send_codec_ctrl(CodecCommandBuffer, 2);

	CodecCommandBuffer[0] = CODEC_MAP_PLAYBACK_CTRL1;
	CodecCommandBuffer[1] = 0x70; //Headphone Gain 0.647 (default)
	send_codec_ctrl(CodecCommandBuffer, 2);
	...
	CodecCommandBuffer[1] = 0x07; //ok
	send_codec_ctrl(CodecCommandBuffer, 2);

	//from here starting mods for passthrough

	CodecCommandBuffer[0] = 0x08;
	CodecCommandBuffer[1] = 0x01;//Select A1 Source Passthrough
	send_codec_ctrl(CodecCommandBuffer, 2);

	CodecCommandBuffer[0] = 0x09;
	CodecCommandBuffer[1] = 0x01;//Select B1 Source Passthrough
	send_codec_ctrl(CodecCommandBuffer, 2);

	// end mods for passthrough

	CodecCommandBuffer[0] = 0x0A; //Analog appear directly
	CodecCommandBuffer[1] = 0x00;
	send_codec_ctrl(CodecCommandBuffer, 2);

	CodecCommandBuffer[0] = 0x0E;
	CodecCommandBuffer[1] = 0xC0 ;//Passthrough analog enabled
	send_codec_ctrl(CodecCommandBuffer, 2);


	CodecCommandBuffer[0] = 0x14;
	CodecCommandBuffer[1] = 0x00 ;//Passthrough Volume A +12dB
	send_codec_ctrl(CodecCommandBuffer, 2);

	CodecCommandBuffer[0] = 0x15;
	CodecCommandBuffer[1] = 0x00 ;//Passthrough Volume B +12dB
	send_codec_ctrl(CodecCommandBuffer, 2);
...
void start_codec_ugly()
{
 int32_t init_Count_bad =0;
