...
#include "arm_const_structs.h"  //why does this not work??????? Had to include the tables and constructs????
...
const float32_t nr_alpha = 0.995f;
	...
	static float32_t 					FFT_buffer[NR_FFT_L_2 * 2];
	static float32_t 					Nest[NR_FFT_L_2 / 2][2]; // noise estimates for the current and the last FFT frame
	// float32_t 					vk; // saved 0.24kbytes
	static float32_t 					SNR_prio[NR_FFT_L_2 / 2];
	static float32_t 					SNR_post[NR_FFT_L_2 / 2];
	// static float32_t 					SNR_post_pos; // saved 0.24kbytes
	...
	static float32_t					ax;
	static float32_t					ap;
	static float32_t					xih1;
	static float32_t					xih1r;
	static float32_t					pfac;
	...
	static float32_t			power_threshold = 0.75;
	//int16_t						power_threshold_int;
	static int16_t 	asnr=30;
...
float32_t ph1y[NR_FFT_SIZE];
    ...
    // calculation is performed in-place the FFT_buffer [re, im, re, im, re, im . . .]
      ...
      if(nr_first_time == 2)
      {
 		  for(int bindx = 0; bindx < NR_FFT_L_2 / 2; bindx++)
		  {
			  Nest[bindx][0] = Nest[bindx][0] + 0.05* X[bindx][0];// we do it 20 times to average over 20 frames for app. 100ms only on NR_on/bandswitch/modeswitch,...
			  xt[bindx] = psini * Nest[bindx][0];
		  }
		  NR_init_counter++;
